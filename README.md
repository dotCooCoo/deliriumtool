...
After `npm run build`, the contents of `dist/` are a self-contained static site. You can also open `dist/index.html` directly; it works offline.
...
The unit suite pins every clinical cut-point, band, and dose string to its cited value. A wrong threshold is treated as a patient-safety bug. The Playwright suite covers keyboard operation, screen-reader semantics, print/PDF output, and visual regressions.
