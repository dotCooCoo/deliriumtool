...
| 2026-07-15 | §7.1 pCAM administration details | Letters task now reads "in an even tone, without stopping" (no per-second rate); the Feature-4 second command step is a plain alternative ("other hand" or "add one more finger") per the pCAM instruction tool | The "about one per second" rate and the cannot-use-both-hands conditional came from the adult worksheet, not the cited pediatric instrument | pCAM instruction tool | V | [CCO — pending] | [x] |
